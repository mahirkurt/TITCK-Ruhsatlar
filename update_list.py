...
def process_generic_file(filename, sheet_name, output_filename, header_row, column_map, dtype=None, skip_footer=0):
    ...
    try:
        read_args = {"sheet_name": sheet_name, "header": header_row, "dtype": dtype}
        if skip_footer:
...
def process_ruhsatli_urunler():
    ...
    logging.info(f"'ruhsatli_ilaclar_listesi.xlsx' -> 'RUHSATLI ÜRÜNLER LİSTESİ' sayfası işleniyor...")
    try:
        df = pd.read_excel(filepath, sheet_name="RUHSATLI ÜRÜNLER LİSTESİ", header=5, dtype={'BARKOD': str, 'RUHSAT NUMARASI': str})
        existing_cols = [col for col in column_map.keys() if col in df.columns]
        df = df[existing_cols]
        df.rename(columns=column_map, inplace=True)
        df.dropna(how='all', inplace=True)
...
def process_skrs_aktif_urunler():
    ...
    return process_generic_file("skrs_erecete_listesi.xlsx", "AKTİF ÜRÜNLER LİSTESİ", "skrs_aktif_urunler.jsonl", 2, column_map, {'BARKOD': str})
