...
def process_generic_file(input_filename, sheet_name, output_filename, header_row, column_map, text_columns=None, skip_footer=0):
    """Genel Excel sayfalarını okur, temel temizlik yapar ve JSONL olarak kaydeder."""
    ...
    try:
        ...
        if not existing_cols:
            logging.error(f"'{sheet_name}' sayfasında belirtilen sütunlardan hiçbiri bulunamadı. Lütfen column_map'i kontrol edin.")
            return False
...
def process_ruhsatli_urunler():
    ...
    logging.info(f"'{input_filename}' -> '{sheet_name}' sayfası işleniyor...")
    try:
        df = pd.read_excel(raw_file_path, sheet_name=sheet_name, header=5, dtype={'BARKOD': str, 'RUHSAT NUMARASI': str})
        ...
        processed_file_path = PROCESSED_DATA_DIR / output_filename; df.to_json(processed_file_path, orient='records', lines=True, force_ascii=False)
...
def process_skrs_aktif_urunler():
    ...
    logging.info(f"'{input_filename}' -> '{sheet_name}' sayfası işleniyor...")
    try:
        df = pd.read_excel(raw_file_path, sheet_name=sheet_name, header=2, dtype={'Barkod': str})
        existing_cols = [col for col in column_map.keys() if col in df.columns]; df = df[existing_cols]; df.rename(columns=column_map, inplace=True); df.dropna(how='all', inplace=True)
...
def main():
    ...
    # Her dosya için ilgili fonksiyonları çağırıyoruz.
    # process_generic_file kullananlar:
    process_ilac_fiyat_listesi_success = process_generic_file("ilac_fiyat_listesi.xlsx", "YURTDIŞI FİYAT LİSTESİ", "ilac_fiyatlari.jsonl", 0, {'ILAC ADI': 'urun_adi', 'FIRMA ADI': 'firma_adi', 'GERCEK KAYNAK FIYAT (GKF) (€)': 'gkf_eur'})
    process_etkin_maddeler_success = process_generic_file("etkin_madde_listesi.xlsx", "Sheet1", "etkin_maddeler.jsonl", 5, {'ETKİN MADDE': 'etkin_madde_adi', 'KODU': 'basvuru_dosyasi_sayisi'}, skip_footer=1)
    process_skrs_pasif_urunler_success = process_generic_file("skrs_erecete_listesi.xlsx", "PASİF ÜRÜNLER LİSTESİ", "skrs_pasif_urunler.jsonl", 2, {'İlaç Adı': 'urun_adi', 'Barkod': 'barkod', 'ATC Kodu': 'atc_kodu', 'ATC Adı': 'atc_adi', 'Firma Adı': 'firma_adi', 'Reçete Türü': 'recete_turu', 'Pasif Ürünler Listesine Alındığı Tarih': 'listeye_alinma_tarihi'}, {'Barkod': str})
    process_yurtdisi_etkin_maddeler_success = process_generic_file("yurtdisi_etkin_madde_listesi.xlsx", "YD-Etkin madde listesi", "yurtdisi_etkin_maddeler.jsonl", 1, {'ETKİN MADDE ADI': 'etkin_madde', 'HASTALIK/TANI': 'hastalik_tani', 'KISITLAMA': 'kisitlama'})
    
    # Kendine özgü işlemleri olanlar:
    process_ruhsatli_urunler_success = process_ruhsatli_urunler()
    process_skrs_aktif_urunler_success = process_skrs_aktif_urunler()

    results = [
        process_ilac_fiyat_listesi_success,
        process_ruhsatli_urunler_success,
        process_etkin_maddeler_success,
        process_skrs_aktif_urunler_success,
        process_skrs_pasif_urunler_success,
        process_yurtdisi_etkin_maddeler_success
    ]

    if all(results):
        logging.info("===== Tüm Dosyalar Başarıyla İşlendi =====")
    else:
        logging.error("!!! Bazı dosyalar işlenirken hatalar oluştu."); sys.exit(1)
